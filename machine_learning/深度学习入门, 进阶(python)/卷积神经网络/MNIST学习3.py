...
param_path = "/home/yywd/work/machine_learning/深度学习入门, 进阶(python)/卷积神经网络/cnn_params.pkl"
# ========== 训练或加载 ==========
try:
    # 尝试加载保存的模型参数
    ...
    print("训练完成，模型参数已保存到 param_path")
...
def preprocess_image_rpg_style(img_path, flatten=False, normalize=True):
    """RPG风格预处理：彩色背景 & 干扰文字鲁棒"""
    # 1️⃣ 读图
    img = cv2.imread(img_path)
    gray = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)

    # 2️⃣ 转 LAB 或 HSV，增强亮度对比
    lab = cv2.cvtColor(img, cv2.COLOR_BGR2LAB)
    L, A, B = cv2.split(lab)

    # 用亮度通道阈值（更适合彩色背景）
    _, bw = cv2.threshold(L, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    ...
    for c in contours:
        ...
        if area > 50 and 0.2 < aspect < 5:  # 粗略过滤
            candidates.append((c, area))
    if not candidates:
        raise ValueError("没有检测到合适的数字区域！")

    # 取面积最大的候选
    largest_contour = max(candidates, key=lambda x:x[1])[0]
    ...
    digit = cv2.bitwise_and(gray, gray, mask=mask)
...
# ================== 示例 ==================
img_path = "/home/yywd/work/machine_learning/深度学习入门, 进阶(python)/卷积神经网络/my_digit.png"
result = predict_image(networks, img_path, flatten=False)
